...
def _normalize_observation_for_agent(observation: Any) -> dict[str, Any]:
    if hasattr(observation, "model_dump"):
        raw = observation.model_dump()
    elif isinstance(observation, dict):
        raw = observation
    else:
        return {"robot_status": {}, "nearby": {"victims": [], "hazards": []}}

    if "nearby" in raw:
        return raw

    robot_raw = raw.get("robot_status", {}) if isinstance(raw, dict) else {}
    nearby_victims = raw.get("nearby_victims", []) if isinstance(raw, dict) else []
    nearby_hazards = raw.get("nearby_hazards", []) if isinstance(raw, dict) else []

    norm_victims: list[dict[str, Any]] = []
    for item in nearby_victims:
        ...
        if isinstance(pos, dict):
            ...
            x = 0.0
            y = 0.0
        health = str(item.get("health", "healthy")).lower()
        priority = "critical" if health == "critical" else "normal"
        norm_victims.append(
            {
                "id": item.get("victim_id", "unknown"),
                "position": [x, y],
                "priority": priority,
            }
        )

    norm_hazards: list[dict[str, Any]] = []
    for item in nearby_hazards:
        ...
        norm_hazards.append(
            {
                "type": item.get("hazard_type", "structural"),
                "location": [x, y],
                "severity": item.get("severity", 0.0),
            }
        )

    position = robot_raw.get("position", {}) if isinstance(robot_raw, dict) else {}
    ...
    return {
        "robot_status": {
            ...
            "battery_level": float(robot_raw.get("battery_level", 100.0)) if isinstance(robot_raw, dict) else 100.0,
        },
        "nearby": {
            "victims": norm_victims,
            "hazards": norm_hazards,
        },
        "time_remaining": raw.get("time_remaining"),
        "mission_progress": raw.get("mission_progress"),
    }
...
def _run_env(
    ...
    for ep in range(episodes):
        ...
        for _ in range(max_steps):
            action = agent.act(_normalize_observation_for_agent(obs))
